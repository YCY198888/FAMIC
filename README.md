...
To verify model performance on the Wine and Twitter datasets:

- **Example script**: `python examples/example_usage.py`
- **Evaluation script**: `python scripts/evaluate.py`
- **Jupyter notebook (local)**: Open and run `notebooks/famic_tutorial.ipynb`
- **Google Colab**: Open `notebooks/famic_tutorial_colab.ipynb` — automatically clones the repo and installs dependencies

All datasets, tokenizers, and pretrained weights are downloaded automatically from HuggingFace on first run. No authentication is required.
